...
// HEAVY-optimizer-vs-interpreter differential fuzzer for MEMORY access.
//
// The lite-tier twin (lite_translator/arm64_to_x86_64/
// memory_differential_fuzz_tests.cc) covers the same corpus against the
// single-pass JIT. This file covers the second gear, whose miscompiles are
// typically region-structural: a destructive lowering that clobbers a value the
// register mapping still needs, or a guest-context cache that forwards a store
// at the wrong width. The width-blind context cache that corrupted bcrypt is
// exactly that shape, and it is only observable when memory is compared.
//
// The register-only heavy fuzzer (heavy_differential_fuzz_tests.cc) never emits
// a load, a store or an atomic, and never compares memory --- the same blind
// spot the lite twin describes:
//
...
//
// Design mirrors the lite memory fuzzer so failures reproduce the same way:
...
alignas(16) uint8_t g_scratch_heavy[kScratchBytes];
alignas(16) uint8_t g_seed_mem_heavy[kScratchBytes];
alignas(16) uint8_t g_jit_mem_heavy[kScratchBytes];

class Arm64HeavyMemoryDifferentialFuzz : public ::testing::Test {
 protected:
  ThreadState state_{};
  uint64_t seed_ = 0;
  ...
  static uint64_t ScratchBase() { return static_cast<uint64_t>(ToGuestAddr(g_scratch_heavy)); }
  ...
  void SeedScratch() {
    for (size_t i = 0; i < kScratchBytes; i += 8) {
      uint64_t v = Rnd64();
      memcpy(g_seed_mem_heavy + i, &v, 8);
    }
    memcpy(g_scratch_heavy, g_seed_mem_heavy, kScratchBytes);
  }
  ...
  Result RunDifferential(const uint32_t* code, int n, const InitState& in, std::string* desc) {
    ...
    // Heavy leg. A region is compared only when the heavy tier translated it
    // WHOLE: a bail falls back to lite/interpreter on-device and so cannot be a
    // heavy miscompile source.
    memcpy(g_scratch_heavy, g_seed_mem_heavy, kScratchBytes);
    ApplyInit(in);
    ...
    if (!ok || stop != code_end || num != static_cast<size_t>(n)) return kDeclined;
    ...
    memcpy(g_jit_mem_heavy, g_scratch_heavy, kScratchBytes);
  ...
  bool Compare(const FullState& a,
    ...
    if ((a.flags & kNZCVMask) != (b.flags & kNZCVMask)) {
      snprintf(buf, sizeof(buf), "NZCV HEAVY=0x%04x INTERP=0x%04x region:", a.flags & kNZCVMask,
               b.flags & kNZCVMask);
      *desc = std::string(buf) + RegionStr(code, n);
      return false;
    }
    ...
    if (memcmp(g_jit_mem_heavy, g_scratch_heavy, kScratchBytes) != 0) {
      for (size_t i = 0; i < kScratchBytes; i++) {
        if (g_jit_mem_heavy[i] != g_scratch_heavy[i]) {
          snprintf(buf, sizeof(buf),
                   "memory[+%zu] HEAVY=0x%02x INTERP=0x%02x (offset from base %+d) region:", i,
                   g_jit_mem_heavy[i], g_scratch_heavy[i], static_cast<int>(i) - static_cast<int>(kWindowBase));
          *desc = std::string(buf) + RegionStr(code, n);
          return false;
        }
      }
...
TEST_F(Arm64HeavyMemoryDifferentialFuzz, EncodersMatchKnownEncodings) {
...
TEST_F(Arm64HeavyMemoryDifferentialFuzz, ScalarLoadStoreImm) {
  Seed(0xE30A1A5ULL);
  ...
  fprintf(stderr, "heavy ScalarLoadStoreImm: compared=%d/%d\n", compared, iters);
...
TEST_F(Arm64HeavyMemoryDifferentialFuzz, TaggedBaseAddressIsIgnored) {
  Seed(0x7B1A5EA5ULL);
  int iters = 3000 * FuzzScale();
  int compared = 0;
  for (int i = 0; i < iters; i++) {
  ...
  fprintf(stderr, "heavy TaggedBaseAddressIsIgnored: compared=%d/%d\n", compared, iters);
  EXPECT_GT(compared, 0);
}

// LDP/STP including LDPSW, whose sign-extension wedged a real Qt fill loop.
TEST_F(Arm64HeavyMemoryDifferentialFuzz, LoadStorePair) {
  Seed(0xBA12A5ULL);
  int iters = 3000 * FuzzScale();
  int compared = 0;
  for (int i = 0; i < iters; i++) {
  ...
  fprintf(stderr, "heavy LoadStorePair: compared=%d/%d\n", compared, iters);
  EXPECT_GT(compared, 0);
}

// Register-offset addressing, sweeping the extend option and the scale bit.
TEST_F(Arm64HeavyMemoryDifferentialFuzz, RegisterOffsetAddressing) {
  Seed(0x8E60FFA5ULL);
  int iters = 3000 * FuzzScale();
  int compared = 0;
  for (int i = 0; i < iters; i++) {
  ...
  fprintf(stderr, "heavy RegisterOffsetAddressing: compared=%d/%d\n", compared, iters);
  EXPECT_GT(compared, 0);
}

// Pre/post-index, which write the base register back as well as accessing
// memory -- both effects are compared.
TEST_F(Arm64HeavyMemoryDifferentialFuzz, PrePostIndexWriteback) {
  Seed(0x1D00A5ULL);
  int iters = 3000 * FuzzScale();
  int compared = 0;
  for (int i = 0; i < iters; i++) {
  ...
  fprintf(stderr, "heavy PrePostIndexWriteback: compared=%d/%d\n", compared, iters);
  EXPECT_GT(compared, 0);
}
...
TEST_F(Arm64HeavyMemoryDifferentialFuzz, SimdLoadStoreImm) {
  Seed(0x51D0A5ULL);
  int iters = 3000 * FuzzScale();
  int compared = 0;
  for (int i = 0; i < iters; i++) {
  ...
  fprintf(stderr, "heavy SimdLoadStoreImm: compared=%d/%d\n", compared, iters);
  EXPECT_GT(compared, 0);
}
...
// compared.
TEST_F(Arm64HeavyMemoryDifferentialFuzz, LseAtomics) {
  Seed(0xA701CA5ULL);
  int iters = 3000 * FuzzScale();
  int compared = 0;
  for (int i = 0; i < iters; i++) {
  ...
  fprintf(stderr, "heavy LseAtomics: compared=%d/%d\n", compared, iters);
  ...
  fprintf(stderr, "heavy LseAtomics: (declines are legitimate)\n");
}
...
TEST_F(Arm64HeavyMemoryDifferentialFuzz, MixedMemoryRegions) {
  Seed(0xBE6100A5ULL);
  ...
  fprintf(stderr, "heavy MixedMemoryRegions: compared=%d/%d\n", compared, iters);
  EXPECT_GT(compared, 0);
}
